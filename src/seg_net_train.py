...
from seg_net_model import SegNet
from seg_net_utils import init, read_config_file, get_tf_dataset
...
def batch_train(FLAGS):
    print("Initializing.............................")
    model_dir = FLAGS.model_dir + FLAGS.model_to_use + "_" + str(FLAGS.num_epochs)
    init(model_dir)
    ...
    seg_net.vgg16_encoder(features)
    print("vgg-16 encoder built")

    if FLAGS.model_to_use == "bilinear":
        seg_net.segnet_bilinear()
        print("segnet with bilinear upsampling decoder built")
    elif FLAGS.model_to_use == "nn":
        seg_net.segnet_nn()
        print("segnet with nn upsampling decoder built")
    elif FLAGS.model_to_use == "tr_conv":
        seg_net.segnet_tr_conv()
        print("segnet with transposed convolution upsampling decoder built")
    else:
        seg_net.segnet_bayesian()
        print("bayesian segnet with transposed convolution upsampling decoder built")

    logits = seg_net.logits
    ...
    for epoch in range(FLAGS.num_epochs):
        ...
        print(f"Epoch : {epoch + 1} / {FLAGS.num_epochs}, time taken : {ti} sec.")
        print(f"training loss : {temp_train_loss_per_epoch / num_batches_train}, \
            validation loss : {temp_valid_loss_per_epoch / num_batches_valid}, \
            validation accuracy : {temp_valid_acc_per_epoch}\n"
        )
    ...
    np.save(os.path.join(
        os.getcwd(), model_dir, FLAGS.model_metrics), (losses_dict)
    )
...
def main():
    ...
    parser.add_argument("--model_to_use", default=model_to_use, type=str,
        choices=["bilinear", "nn", "tr_conv", "bayesian"], help="model to use")
