...
from seg_net_utils import read_config_file, init
...
def infer(FLAGS):
    ...
    seg_net.vgg16_encoder(images_features_infer)
    ...
    print(f"Running inference on following data : {images_dir_infer}\n")
    ...
    tf.train.Saver().restore(ss, os.path.join(
        os.getcwd(), model_dir, FLAGS.model_file + "-" + str(FLAGS.which_checkpoint_model))
    )
    ...
    for img_file in list_images_infer:
        ...
        cv2.imwrite(os.path.join(
            os.getcwd(), model_dir, labels_dir, "label_" + img_file.split("/")[-1]), labels_predicted
        )
